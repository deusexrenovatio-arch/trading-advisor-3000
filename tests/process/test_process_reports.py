...
def test_governance_dashboard_report(tmp_path: Path) -> None:
    ...
    assert "orchestration_quality" not in payload["sections"]
    ...
    assert "| orchestration_quality |" not in markdown
    assert "avg_score=" not in markdown
...
def test_process_and_kpi_reports(tmp_path: Path) -> None:
    ...
    assert result_process_json.returncode == 0, (
        result_process_json.stdout + "\n" + result_process_json.stderr
    )
