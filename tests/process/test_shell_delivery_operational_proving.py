...
from run_shell_delivery_operational_proving import (  # noqa: E402
    ShellDeliveryProvingStep,
    build_shell_delivery_operational_proving_plan,
    run_shell_delivery_operational_proving_plan,
)


def test_shell_delivery_plan_has_expected_lanes_and_session_policy() -> None:
...
def test_shell_delivery_run_stops_on_first_failed_step(tmp_path: Path) -> None:
    ...
    exit_code, report = run_shell_delivery_operational_proving_plan(
        plan=steps,
        report_path=tmp_path / "shell-delivery-run.json",
        dry_run=False,
        runner=_runner,
...
def test_shell_delivery_run_fails_on_missing_dashboard_artifacts(tmp_path: Path) -> None:
    exit_code, report = run_shell_delivery_operational_proving_plan(
        plan=[],
        report_path=tmp_path / "shell-delivery-artifacts.json",
        dry_run=False,
...
def test_shell_delivery_run_fails_on_stale_dashboard_artifacts(tmp_path: Path) -> None:
    stale_artifact = tmp_path / "dashboard-stale.json"
    stale_artifact.write_text('{"stale": true}\n', encoding="utf-8")
    exit_code, report = run_shell_delivery_operational_proving_plan(
        plan=[],
        report_path=tmp_path / "shell-delivery-stale.json",
        dry_run=False,
...
def test_shell_delivery_cli_dry_run_rejects_output_without_opt_in(tmp_path: Path) -> None:
    output = tmp_path / "shell-delivery-dry-run.json"
...
def test_shell_delivery_cli_dry_run_writes_report_when_opted_in(tmp_path: Path) -> None:
    output = tmp_path / "shell-delivery-dry-run.json"
    result = subprocess.run(
        [
            sys.executable,
