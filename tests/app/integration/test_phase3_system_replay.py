...
def test_phase3_integrated_replay_produces_forward_and_analytics_outcomes(tmp_path: Path) -> None:
    ...
    assert report["runtime_report"]["accepted"] == report["signal_candidates"]
    assert report["runtime_report"]["published"] == report["signal_candidates"]
    assert report["forward_observations"] == report["signal_candidates"]
