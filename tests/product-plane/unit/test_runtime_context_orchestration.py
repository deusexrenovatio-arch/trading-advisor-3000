...
def test_runtime_context_orchestration_runtime_pipeline_uses_context_provider_registry_in_replay_path() -> None:
    ...
    report = stack.runtime_engine.replay_candidates([_candidate(signal_id="SIG-SCALEUP-CONTEXT-1")])
...
def test_runtime_context_orchestration_runtime_pipeline_handles_missing_context_providers_without_rejection() -> None:
    stack = build_runtime_stack(telegram_channel="@ta3000_signals")
    stack.strategy_registry.register(
        StrategyVersion(
            strategy_version_id="ma-cross-v1",
    ...
    report = stack.runtime_engine.replay_candidates([_candidate(signal_id="SIG-SCALEUP-CONTEXT-EMPTY-1")])
    context_events = [item for item in stack.signal_store.list_signal_events() if item.event_type == "signal_context"]
