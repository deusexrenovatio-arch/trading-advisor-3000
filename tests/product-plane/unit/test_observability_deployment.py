...
def test_observability_observability_files_exist() -> None:
    expected = {
        ...
        OBSERVABILITY_DIR / "grafana" / "dashboards" / "ta3000-observability-overview.json",
    }
    for path in expected:
        assert path.exists(), f"missing observability artifact: {path}"


def test_observability_observability_compose_wires_required_services() -> None:
    ...
    assert "promtail-data:/var/lib/promtail" in compose
...
def test_observability_prometheus_and_loki_configs_have_expected_smoke_targets() -> None:
    ...
    assert "filename: /var/lib/promtail/positions.yaml" in promtail


def test_observability_dashboard_is_valid_json_and_contains_key_panels() -> None:
    dashboard_path = OBSERVABILITY_DIR / "grafana" / "dashboards" / "ta3000-observability-overview.json"
    dashboard = json.loads(_read(dashboard_path))
    assert dashboard["title"] == "TA3000 Observability Overview"
...
def test_observability_runtime_artifacts_are_parseable() -> None:
