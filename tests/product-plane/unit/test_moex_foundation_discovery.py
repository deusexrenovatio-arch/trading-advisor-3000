...
def test_discover_coverage_tracks_native_source_identity() -> None:
    ...
    assert len(coverage) == len(active_mappings) * 4
    assert {row.source_interval for row in coverage} == {1, 7, 24, 60}
...
def test_discover_coverage_respects_selected_universe_scope() -> None:
    ...
    assert {row.internal_id for row in coverage} == {"FUT_BR"}
...
def test_discover_coverage_uses_dedicated_contract_discovery_lookback_window() -> None:
    ...
    coverage = discover_coverage(
        client=client,
        universe=[row for row in universe if row.internal_id == "FUT_BR"],
        mappings=mappings,
        timeframes={"5m", "15m"},
    ...
    assert coverage
    assert client.trade_dates
    assert min(client.trade_dates) >= date(2026, 3, 4)
    assert max(client.trade_dates) <= date(2026, 4, 3)
