...
def test_recovery_and_idempotency_submit_reuses_existing_ack_without_duplicate_side_effects() -> None:
    sidecar, sync, engine = _build_engine()
    intent = _live_intent(intent_id="INT-OPS-IDEMP-1")
...
def test_recovery_and_idempotency_recovery_plan_contains_actions_for_quantity_mismatch() -> None:
    sidecar, _, engine = _build_engine()
    intent = _live_intent(intent_id="INT-OPS-RECOVERY-1", qty=1)
...
def test_recovery_and_idempotency_recovery_plan_maps_sync_incidents_without_falling_back_to_uncovered() -> None:
    sidecar, _, engine = _build_engine()
    intent = _live_intent(intent_id="INT-OPS-RECOVERY-2")
