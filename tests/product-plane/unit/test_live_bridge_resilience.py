...
def test_live_bridge_resilience_submit_retries_and_succeeds_before_budget_exhaustion() -> None:
...
def test_live_bridge_resilience_submit_raises_retry_exhausted_when_budget_is_exceeded() -> None:
    sidecar = StockSharpSidecarStub()
    ...
    with pytest.raises(LiveExecutionRetryExhaustedError) as exc_info:
        bridge.submit_order_intent(
            _live_intent("INT-OPS-RETRY-FAIL"),
...
def test_live_bridge_resilience_live_submit_is_blocked_when_secrets_policy_is_not_satisfied() -> None:
    bridge = LiveExecutionBridge(
        sidecar=StockSharpSidecarStub(),
    ...
    with pytest.raises(LiveExecutionDisabledError):
        bridge.submit_order_intent(
            _live_intent("INT-OPS-MISSING-SECRET"),
...
def test_live_bridge_resilience_bridge_health_redacts_secret_values() -> None:
    bridge = LiveExecutionBridge(
        sidecar=StockSharpSidecarStub(),
        flags=_full_live_flags(enforce_live_secrets=True),
        retry_policy=LiveExecutionRetryPolicy(max_attempts=2, backoff_seconds=0.0),
