...
def test_phase7_live_bridge_accepts_custom_registered_adapter() -> None:
    ...
    assert ack["external_order_id"] == "finam-direct-sim-INT-PHASE7-ADAPTER-1"


def test_phase7_live_bridge_rejects_adapter_without_transport_binding() -> None:
    catalog = default_execution_adapter_catalog()
    catalog.register(
        ExecutionAdapterSpec(
    ...
    with pytest.raises(AdapterTransportNotConfiguredError):
        bridge.submit_order_intent(
            _intent(
                intent_id="INT-PHASE7-ADAPTER-UNBOUND",
...
def test_phase7_live_bridge_routes_operations_and_streams_per_adapter_transport() -> None:
    catalog = default_execution_adapter_catalog()
    catalog.register(
        ExecutionAdapterSpec(
            adapter_id="finam-direct-sim",
    ...
    sidecar_ack = bridge.submit_order_intent(
        _intent(
            intent_id="INT-PHASE7-SIDECAR-1",
