...
from trading_advisor_3000.product_plane.data_plane.moex.operational_hardening import (
...
def test_operational_hardening_scheduler_policy_rejects_missing_mandatory_qc_job(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="missing mandatory required jobs"):
        load_operational_scheduler_policy(policy_path)


def test_operational_hardening_blocks_when_monitoring_freshness_metric_missing(tmp_path: Path) -> None:
    raw_ingest, canonicalization, reconciliation, raw_ingest_acceptance, canonicalization_acceptance, reconciliation_acceptance = _write_phase_reports(tmp_path / "reports")
    ...
    report = run_moex_operational_hardening(
        ...
        run_id="operational-hardening-missing-freshness",
    )
...
def test_operational_hardening_blocks_when_recovery_drill_requires_manual_cleanup(tmp_path: Path) -> None:
    raw_ingest, canonicalization, reconciliation, raw_ingest_acceptance, canonicalization_acceptance, reconciliation_acceptance = _write_phase_reports(tmp_path / "reports")
    scheduler_policy = _write_yaml(tmp_path / "scheduler.yaml", _scheduler_policy_payload(include_qc=True))
    monitoring_policy = _write_yaml(tmp_path / "monitoring.yaml", _monitoring_policy_payload())
    scheduler_evidence = _write_scheduler_evidence(tmp_path / "inputs" / "scheduler.json")
    ...
    report = run_moex_operational_hardening(
        ...
        recovery_drill_source_path=recovery_evidence,
        output_dir=tmp_path / "operational_hardening",
        run_id="operational-hardening-manual-cleanup",
    )
...
def test_operational_hardening_blocks_when_monitoring_dashboards_use_example_urls(tmp_path: Path) -> None:
    raw_ingest, canonicalization, reconciliation, raw_ingest_acceptance, canonicalization_acceptance, reconciliation_acceptance = _write_phase_reports(tmp_path / "reports")
    scheduler_policy = _write_yaml(tmp_path / "scheduler.yaml", _scheduler_policy_payload(include_qc=True))
    monitoring_policy = _write_yaml(tmp_path / "monitoring.yaml", _monitoring_policy_payload())
    scheduler_evidence = _write_scheduler_evidence(tmp_path / "inputs" / "scheduler.json")
    ...
    report = run_moex_operational_hardening(
        ...
        recovery_drill_source_path=recovery_evidence,
        output_dir=tmp_path / "operational_hardening",
        run_id="operational-hardening-placeholder-dashboard",
    )
...
def test_operational_hardening_blocks_when_scheduler_source_provenance_missing(tmp_path: Path) -> None:
    raw_ingest, canonicalization, reconciliation, raw_ingest_acceptance, canonicalization_acceptance, reconciliation_acceptance = _write_phase_reports(tmp_path / "reports")
    scheduler_policy = _write_yaml(tmp_path / "scheduler.yaml", _scheduler_policy_payload(include_qc=True))
    monitoring_policy = _write_yaml(tmp_path / "monitoring.yaml", _monitoring_policy_payload())
    scheduler_evidence = _write_scheduler_evidence(tmp_path / "inputs" / "scheduler.json")
    ...
    report = run_moex_operational_hardening(
        ...
        recovery_drill_source_path=recovery_evidence,
        output_dir=tmp_path / "operational_hardening",
        run_id="operational-hardening-missing-source-provenance",
    )
...
def test_operational_hardening_blocks_when_monitoring_immutable_export_path_missing(tmp_path: Path) -> None:
    raw_ingest, canonicalization, reconciliation, raw_ingest_acceptance, canonicalization_acceptance, reconciliation_acceptance = _write_phase_reports(tmp_path / "reports")
    scheduler_policy = _write_yaml(tmp_path / "scheduler.yaml", _scheduler_policy_payload(include_qc=True))
    monitoring_policy = _write_yaml(tmp_path / "monitoring.yaml", _monitoring_policy_payload())
    scheduler_evidence = _write_scheduler_evidence(tmp_path / "inputs" / "scheduler.json")
    ...
    report = run_moex_operational_hardening(
        ...
        recovery_drill_source_path=recovery_evidence,
        output_dir=tmp_path / "operational_hardening",
        run_id="operational-hardening-missing-immutable-export-path",
    )
...
def test_operational_hardening_blocks_when_recovery_immutable_export_hash_mismatch(tmp_path: Path) -> None:
    raw_ingest, canonicalization, reconciliation, raw_ingest_acceptance, canonicalization_acceptance, reconciliation_acceptance = _write_phase_reports(tmp_path / "reports")
    scheduler_policy = _write_yaml(tmp_path / "scheduler.yaml", _scheduler_policy_payload(include_qc=True))
    monitoring_policy = _write_yaml(tmp_path / "monitoring.yaml", _monitoring_policy_payload())
    scheduler_evidence = _write_scheduler_evidence(tmp_path / "inputs" / "scheduler.json")
    ...
    report = run_moex_operational_hardening(
        ...
        recovery_drill_source_path=recovery_evidence,
        output_dir=tmp_path / "operational_hardening",
        run_id="operational-hardening-immutable-export-sha-mismatch",
    )
...
def test_operational_hardening_filters_template_real_bindings_from_prior_phases(tmp_path: Path) -> None:
    ...
    report = run_moex_operational_hardening(
        ...
        recovery_drill_source_path=recovery_evidence,
        output_dir=tmp_path / "operational_hardening",
        run_id="operational-hardening-template-bindings-filter",
    )
...
def test_operational_hardening_blocks_when_prior_phase_acceptance_not_closed(tmp_path: Path) -> None:
    raw_ingest, canonicalization, reconciliation, raw_ingest_acceptance, canonicalization_acceptance, reconciliation_acceptance = _write_phase_reports(
        tmp_path / "reports"
    )
    _write_acceptance_report(canonicalization_acceptance, verdict="BLOCKED")
    ...
    report = run_moex_operational_hardening(
        ...
        recovery_drill_source_path=recovery_evidence,
        output_dir=tmp_path / "operational_hardening",
        run_id="operational-hardening-prior-acceptance-blocked",
    )
...
def test_operational_hardening_allows_release_decision_on_full_live_real_bundle(tmp_path: Path) -> None:
    raw_ingest, canonicalization, reconciliation, raw_ingest_acceptance, canonicalization_acceptance, reconciliation_acceptance = _write_phase_reports(tmp_path / "reports")
    scheduler_policy = _write_yaml(tmp_path / "scheduler.yaml", _scheduler_policy_payload(include_qc=True))
    monitoring_policy = _write_yaml(tmp_path / "monitoring.yaml", _monitoring_policy_payload())
    scheduler_evidence = _write_scheduler_evidence(tmp_path / "inputs" / "scheduler.json")
    ...
    report = run_moex_operational_hardening(
        ...
        recovery_drill_source_path=recovery_evidence,
        output_dir=tmp_path / "operational_hardening",
        run_id="operational-hardening-pass",
    )
