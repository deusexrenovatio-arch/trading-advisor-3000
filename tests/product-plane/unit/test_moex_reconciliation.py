...
def _write_canonicalization_inputs(base: Path) -> tuple[Path, Path]:
        ...
        provenance.append(
            {
                ...
                "source_timeframe": "1m",
                "source_interval": 1,
                "source_run_id": "raw-ingest-pass",
                "source_ingest_run_id": "raw-ingest-pass",
                "source_row_count": 5,
                ...
                "build_run_id": "canonicalization-run",
...
def _policy_path() -> Path:
    return Path("configs/moex_reconciliation/reconciliation_thresholds.v1.yaml")
...
def test_reconciliation_threshold_policy_rejects_missing_lag_class_dimension(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="missing mandatory dimensions"):
        load_reconciliation_threshold_policy(broken)


def test_reconciliation_blocks_hard_drift_without_degraded_publish(tmp_path: Path) -> None:
    bars_path, provenance_path = _write_canonicalization_inputs(tmp_path / "canonicalization")
    ...
    report = run_moex_reconciliation(
        ...
        run_id="reconciliation-hard-block",
...
def test_reconciliation_blocks_when_alert_simulation_not_executed(
    tmp_path: Path,
    ...
    report = run_moex_reconciliation(
        ...
        threshold_policy_path=_policy_path(),
        mapping_registry_path=_mapping_registry_path(),
        output_dir=tmp_path / "reconciliation-no-alerts",
        run_id="reconciliation-alert-missing",
...
def test_reconciliation_rejects_missing_proof_critical_metadata_fields(tmp_path: Path) -> None:
    bars_path, provenance_path = _write_canonicalization_inputs(tmp_path / "canonicalization")
    ...
    with pytest.raises(ValueError, match="archive_batch_id"):
        run_moex_reconciliation(
            ...
            threshold_policy_path=_policy_path(),
            mapping_registry_path=_mapping_registry_path(),
            output_dir=tmp_path / "reconciliation-metadata-missing",
            run_id="reconciliation-metadata-missing",
            allow_degraded_publish=False,
        )


def test_reconciliation_rejects_aliased_timestamp_metadata(tmp_path: Path) -> None:
    bars_path, provenance_path = _write_canonicalization_inputs(tmp_path / "canonicalization")
    ...
    with pytest.raises(ValueError, match="source_ts_utc"):
        run_moex_reconciliation(
            ...
            threshold_policy_path=_policy_path(),
            mapping_registry_path=_mapping_registry_path(),
            output_dir=tmp_path / "reconciliation-metadata-aliased",
            run_id="reconciliation-metadata-aliased",
            allow_degraded_publish=False,
        )
