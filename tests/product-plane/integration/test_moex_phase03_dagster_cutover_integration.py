...
def test_phase03_dagster_cutover_materializes_route_and_emits_recovery_artifacts(tmp_path: Path) -> None:
    ...
    assert "phase03-int-nightly-1" in run_ids
    assert "phase03-int-nightly-2" in run_ids
    assert "phase03-int-repair-1" in run_ids
    assert "phase03-int-backfill-1" in run_ids
