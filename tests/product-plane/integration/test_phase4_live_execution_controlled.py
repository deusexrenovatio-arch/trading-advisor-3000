...
def test_phase4_controlled_live_cycle_syncs_and_reconciles_cleanly() -> None:
...
def test_phase4_controlled_live_cycle_rejects_live_when_flags_disabled() -> None:
    sidecar = StockSharpSidecarStub()
    bridge = LiveExecutionBridge(
        sidecar=sidecar,
        flags=LiveExecutionFeatureFlags(),
...
def test_phase4_replace_flow_is_supported_end_to_end() -> None:
    sidecar = StockSharpSidecarStub()
    bridge = LiveExecutionBridge(
        sidecar=sidecar,
        flags=LiveExecutionFeatureFlags(
