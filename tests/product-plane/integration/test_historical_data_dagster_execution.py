...
def test_historical_data_dagster_disprover_fails_on_missing_source_fixture(tmp_path: Path) -> None:
...
def test_historical_data_dagster_disprover_fails_for_metadata_only_definitions(monkeypatch, tmp_path: Path) -> None:
    ...
    try:
        ...
        assert "historical_data_proof_materialization_job" in message
    else:
