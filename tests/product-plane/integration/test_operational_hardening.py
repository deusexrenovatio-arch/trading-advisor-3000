...
def test_operational_hardening_retry_exhaustion_does_not_leave_ghost_intent_state() -> None:
    sidecar = StockSharpSidecarStub()
    ...
    with pytest.raises(LiveExecutionRetryExhaustedError):
        engine.submit_intents(
            [_live_intent(intent_id="INT-OPS-FAILURE-1")],
...
def test_operational_hardening_hardened_cycle_surfaces_recovery_plan_for_non_happy_state() -> None:
    sidecar = StockSharpSidecarStub()
    _, engine = _build_engine(sidecar=sidecar, retry_max_attempts=3)
    report = engine.run_hardened_cycle(
        intents=[_live_intent(intent_id="INT-OPS-HARDENED-1")],
...
def test_operational_hardening_controlled_live_replay_reuses_submission_ack_idempotently() -> None:
    sidecar = StockSharpSidecarStub()
    sync, engine = _build_engine(sidecar=sidecar, retry_max_attempts=3)
    intent = _live_intent(intent_id="INT-OPS-IDEMP-2")
