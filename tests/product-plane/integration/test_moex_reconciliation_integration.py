...
from trading_advisor_3000.product_plane.data_plane.moex import run_moex_reconciliation
...
def _prepare_canonicalization_tables(base: Path) -> tuple[Path, Path]:
...
def test_reconciliation_reconciliation_generates_metrics_and_artifacts(tmp_path: Path) -> None:
    bars_path, provenance_path = _prepare_canonicalization_tables(tmp_path / "canonicalization")
    ...
    report = run_moex_reconciliation(
        ...
        threshold_policy_path=Path("configs/moex_reconciliation/reconciliation_thresholds.v1.yaml"),
        mapping_registry_path=Path("configs/moex_foundation/instrument_mapping_registry.v1.yaml"),
        output_dir=tmp_path / "reconciliation",
        run_id="reconciliation-int-pass",
