...
def test_phase04_generates_release_bundle_and_checklist(tmp_path: Path) -> None:
    phase01, phase02, phase03, phase01_acceptance, phase02_acceptance, phase03_acceptance = _phase_reports(tmp_path / "reports")
...
def test_phase04_denies_release_when_open_p1_exists(tmp_path: Path) -> None:
    phase01, phase02, phase03, phase01_acceptance, phase02_acceptance, phase03_acceptance = _phase_reports(tmp_path / "reports")
    scheduler_policy = _write_yaml(tmp_path / "config" / "scheduler.yaml", _scheduler_policy())
    monitoring_policy = _write_yaml(tmp_path / "config" / "monitoring.yaml", _monitoring_policy())
    scheduler_evidence = _scheduler_evidence(tmp_path / "inputs" / "scheduler.json")
    ...
    report = run_phase04_production_hardening(
        ...
        output_dir=tmp_path / "phase04",
        run_id="phase04-int-defect-block",
    )
...
def test_phase04_integration_blocks_when_monitoring_provenance_hash_mismatch(tmp_path: Path) -> None:
    phase01, phase02, phase03, phase01_acceptance, phase02_acceptance, phase03_acceptance = _phase_reports(tmp_path / "reports")
    scheduler_policy = _write_yaml(tmp_path / "config" / "scheduler.yaml", _scheduler_policy())
    monitoring_policy = _write_yaml(tmp_path / "config" / "monitoring.yaml", _monitoring_policy())
    scheduler_evidence = _scheduler_evidence(tmp_path / "inputs" / "scheduler.json")
    ...
    report = run_phase04_production_hardening(
        ...
        defects_source_path=None,
        output_dir=tmp_path / "phase04",
        run_id="phase04-int-monitoring-hash-mismatch",
    )
...
def test_phase04_integration_filters_template_real_bindings(tmp_path: Path) -> None:
    ...
    report = run_phase04_production_hardening(
        ...
        defects_source_path=None,
        output_dir=tmp_path / "phase04",
        run_id="phase04-int-template-binding-filter",
    )
...
def test_phase04_integration_blocks_without_prior_acceptance_closure(tmp_path: Path) -> None:
    phase01, phase02, phase03, phase01_acceptance, phase02_acceptance, phase03_acceptance = _phase_reports(
        tmp_path / "reports"
    )
    _write_acceptance_report(phase03_acceptance, verdict="BLOCKED")
    scheduler_policy = _write_yaml(tmp_path / "config" / "scheduler.yaml", _scheduler_policy())
    monitoring_policy = _write_yaml(tmp_path / "config" / "monitoring.yaml", _monitoring_policy())
    scheduler_evidence = _scheduler_evidence(tmp_path / "inputs" / "scheduler.json")
    monitoring_evidence = _monitoring_evidence(tmp_path / "inputs" / "monitoring.json")
    ...
    report = run_phase04_production_hardening(
        ...
        defects_source_path=None,
        output_dir=tmp_path / "phase04",
        run_id="phase04-int-prior-acceptance-blocked",
    )
