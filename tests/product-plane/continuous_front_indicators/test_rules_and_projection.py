...
def test_delta_lineage_gate_fails_on_hash_membership_mismatch(
    ...
    def fake_read_delta_table_rows(table_path: Path, **_: object) -> list[dict[str, object]]:
        if table_path == input_path:
            return [input_row]
        if table_path == base_path:
            return [base_row]
    ...
    monkeypatch.setattr(pandas_job, "read_delta_table_rows", fake_read_delta_table_rows)
...
def test_delta_lineage_gate_accepts_hash_membership_outside_first_input_sample(
    ...
    def fake_read_delta_table_rows(table_path: Path, **_: object) -> list[dict[str, object]]:
        if table_path == input_path:
            return [unrelated_input_row, matching_input_row]
        if table_path == base_path:
            return [base_row]
        if table_path == derived_path:
            return [derived_row]
