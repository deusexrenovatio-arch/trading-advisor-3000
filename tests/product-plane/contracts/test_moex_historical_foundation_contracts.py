...
def test_raw_ingest_manifest_registers_handoff_contract_boundary() -> None:
...
def test_raw_ingest_schema_fixtures_validate() -> None:
    pairs = {
...
def test_raw_ingest_raw_ingest_report_status_semantics_are_unambiguous() -> None:
    changed_window = [
        {
            "internal_id": "BR-6.26",
            "source_timeframe": "1m",
    ...
    with pytest.raises(ValueError, match="PASS-NOOP requires `changed_windows` to be empty"):
        build_raw_ingest_run_report_v2(
            run_id="raw-ingest-pass2-invalid",
            ingest_till_utc="2026-04-01T10:40:00Z",
            source_rows=2,
            incremental_rows=0,
            deduplicated_rows=2,
...
def test_raw_ingest_parity_manifest_is_deterministic_for_same_windows() -> None:
...
def test_raw_ingest_parity_manifest_rejects_invalid_changed_windows_hash() -> None:
    raw_report = _load_json(FIXTURES / "raw_ingest_run_report.v2.json")
    ...
    with pytest.raises(
        ...
        build_parity_manifest_v1(
            run_id="raw-ingest-parity-invalid-hash",
            raw_ingest_run_report=raw_report,
...
def test_raw_ingest_lease_api_responses_match_contracts(tmp_path: Path) -> None:
