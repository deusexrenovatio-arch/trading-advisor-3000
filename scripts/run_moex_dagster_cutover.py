...
from trading_advisor_3000.product_plane.data_plane.moex import run_moex_dagster_cutover


DEFAULT_OUTPUT_ROOT = Path("artifacts/codex/moex-dagster-cutover")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run MOEX Dagster cutover contour: canonical route graph materialization, "
    ...
    parser.add_argument("--output-root", default=DEFAULT_OUTPUT_ROOT.as_posix(), help="Root folder for Dagster cutover artifacts.")
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Required for governed Dagster cutover staging-real proof."
        ),
    )
    ...
    if not raw_table_raw:
        raise SystemExit("dagster cutover requires --raw-table-path")
    if not raw_report_raw:
        raise SystemExit("dagster cutover requires --raw-ingest-report-path")
    if len(args.nightly_readiness_observed_at_utc) != 2:
        raise SystemExit(
            "dagster cutover requires exactly two --nightly-readiness-observed-at-utc values "
    ...
    if not staging_binding_raw:
        raise SystemExit(
            "governed Dagster cutover requires --staging-binding-report-path "
    ...
    report_path = output_dir / "dagster-cutover-report.json"
    report_path.write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(json.dumps(report, ensure_ascii=False, indent=2))
    if report.get("status") != "PASS":
        raise SystemExit("dagster cutover contour blocked by fail-closed checks")
