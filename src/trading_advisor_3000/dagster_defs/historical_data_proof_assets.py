"""Historical-data Dagster proof assets.

These assets stay in the repository as a tested proof contour. They are not the
authoritative MOEX historical refresh route and should not be treated as the
scheduled production-like MOEX pipeline.
"""
...
def historical_data_proof_asset_specs() -> list[AssetSpec]:
    return [
        AssetSpec(
...
class HistoricalDataProofMaterializationConfig(Config):
    source_path: str
    output_dir: str
...
def raw_market_backfill(config: HistoricalDataProofMaterializationConfig) -> dict[str, object]:
    output_dir = Path(config.output_dir).resolve()
    output_paths = historical_data_proof_output_paths(output_dir)
...
historical_data_proof_materialization_job = define_asset_job(
    name="historical_data_proof_materialization_job",
    selection=AssetSelection.groups("historical_data_proof"),
)


historical_data_proof_definitions = Definitions(
    assets=list(HISTORICAL_DATA_PROOF_ASSETS),
    jobs=[historical_data_proof_materialization_job],
)
...
def assert_historical_data_proof_definitions_executable(definitions: Definitions | None = None) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            "historical-data Dagster definitions are metadata-only or incomplete: "
            "missing executable `historical_data_proof_materialization_job`."
        ) from exc

    expected_nodes = set(HISTORICAL_DATA_PROOF_TABLES)
...
def build_historical_data_proof_definitions() -> Definitions:
    assert_historical_data_proof_definitions_executable(historical_data_proof_definitions)
    return historical_data_proof_definitions


def historical_data_proof_output_paths(output_dir: Path) -> dict[str, str]:
    return {
...
def _resolve_selected_assets(selection: Sequence[str] | None) -> list[str]:
    if selection is None:
        return list(HISTORICAL_DATA_PROOF_TABLES)
    ...
    unknown = [item for item in normalized if item not in HISTORICAL_DATA_PROOF_ASSET_BY_NAME]
    if unknown:
        unknown_text = ", ".join(sorted(unknown))
        raise ValueError(f"unknown historical-data asset selection: {unknown_text}")
    return normalized
...
def _resolve_expected_materialization(selection: Sequence[str]) -> list[str]:
    ...
    def _visit(asset_name: str) -> None:
        if asset_name in resolved:
            return
        for dependency in HISTORICAL_DATA_PROOF_DEPENDENCIES.get(asset_name, tuple()):
            _visit(dependency)
        resolved.add(asset_name)

    for asset_name in selection:
        _visit(asset_name)
    return [table_name for table_name in HISTORICAL_DATA_PROOF_TABLES if table_name in resolved]


def materialize_historical_data_proof_assets(
    *,
    source_path: Path,
    output_dir: Path,
