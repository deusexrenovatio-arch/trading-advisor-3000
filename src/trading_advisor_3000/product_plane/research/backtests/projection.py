...
from trading_advisor_3000.product_plane.research.io import (
    ResearchFrameCache,
...
def project_runtime_candidates(
    *,
    ...
    if ranking_rows is None:
        raise ValueError(
            "projection requires explicit ranking rows from the Delta-backed orchestration layer"
        )
    if not ranking_rows:
    ...
    for ranking_row in selected_rows:
        ...
        derived_indicator_set_version = str(
            ranking_row.get("derived_indicator_set_version", "derived-v1")
        )
        ...
        search_spec = strategy_spec_to_search_spec(
            strategy_spec,
            template_key=str(
                ranking_row.get("template_key", ranking_row.get("strategy_template_id", ""))
                or strategy_spec.signal_builder_key
            ),
        )
        ...
        params = _coerce_json(
            _coerce_json(ranking_row.get("rank_reason_json")).get("parameter_values", {})
        )
        ...
        confidence = _clip_unit(
            (0.65 * float(ranking_row["score_total"]))
            + (0.35 * float(projection["signal_strength_score"]))
        )
        ...
        projected_rows.append(
            {
                ...
                "window_id": str(ranking_row.get("window_ids_json", ["wf-01"])[0])
                if ranking_row.get("window_ids_json")
                else "wf-01",
...
def _optimizer_selection_rows(
    ...
    for trial in trials:
        ...
        if status not in {"completed", "duplicate"} or not bool(
            trial.get("constraints_passed", False)
        ):
            continue
        ...
        matching_runs = runs_by_campaign_param.get(
            (campaign_run_id, param_hash), runs_by_param.get(param_hash, [])
        )
        ...
        for (contract_id, instrument_id, timeframe), group in sorted(grouped_runs.items()):
            ...
            family_key = str(
                representative.get(
                    "family_key", representative.get("strategy_family", study.get("family_key", ""))
                )
            )
            template_key = str(
                study.get("template_key", representative.get("strategy_template_id", ""))
            )
            ...
            selected_rows.append(
                {
                    ...
                    or str(representative.get("campaign_run_id", "")),
                    "backtest_run_id": str(representative.get("backtest_run_id", "")),
                    "representative_backtest_run_id": str(
                        representative.get("backtest_run_id", "")
                    ),
                    "strategy_instance_id": str(
                        representative.get("strategy_instance_id", f"sinst_{param_hash}")
                    ),
                    "strategy_template_id": str(
                        representative.get("strategy_template_id", template_key)
                    ),
                    ...
                    "derived_indicator_set_version": str(
                        representative.get("derived_indicator_set_version", "derived-v1")
                    ),
                    "contract_id": contract_id,
                    "instrument_id": instrument_id,
                    "timeframe": timeframe,
                    ...
                    "objective_score": float(
                        components.get("objective_score", trial.get("value", 0.0)) or 0.0
                    ),
                    "policy_metric_score": float(
                        components.get("objective_score", trial.get("value", 0.0)) or 0.0
                    ),
                    "score_total": float(
                        components.get("score_total", trial.get("value", 0.0)) or 0.0
                    ),
                    "robust_score": float(
                        components.get("score_total", trial.get("value", 0.0)) or 0.0
                    ),
                    ...
                    "rank_reason_json": {
                        ...
                        "window_ids": tuple(
                            str(row.get("window_id", "")) for row in group if row.get("window_id")
                        ),
                    },
                    "window_ids_json": tuple(
                        str(row.get("window_id", "")) for row in group if row.get("window_id")
                    ),
                    "created_at": _created_at(),
                }
...
def _select_rows(
    ...
    *,
    ...
    filtered = [
        ...
        and float(row.get("score_total", row.get("robust_score", 0.0)) or 0.0)
        >= request.min_robust_score
    ]
    if request.backtest_run_id:
        filtered = [
            row
            for row in filtered
            if str(row.get("backtest_run_id", row.get("representative_backtest_run_id", "")))
...
def _sorted_rows(
    rows: list[dict[str, object]], *, selection_policy: str
) -> list[dict[str, object]]:
