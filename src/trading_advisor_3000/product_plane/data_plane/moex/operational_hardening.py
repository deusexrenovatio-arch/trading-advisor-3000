...
ACCEPTANCE_ROUTE_SIGNAL = "acceptance:governed-capability-route"
...
def load_operational_scheduler_policy(path: Path) -> SchedulerPolicy:
    ...
    missing_mandatory = sorted(set(MANDATORY_OPERATIONAL_JOB_IDS) - required_job_ids)
...
def load_operational_monitoring_policy(path: Path) -> MonitoringPolicy:
    payload = yaml.safe_load(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
...
def _validate_capability_status_gate(
    *,
    gate: str,
    name: str,
    report: dict[str, object],
...
def run_moex_operational_hardening(
    *,
    raw_ingest_report_path: Path,
    canonicalization_report_path: Path,
    reconciliation_report_path: Path,
    raw_ingest_acceptance_path: Path,
    canonicalization_acceptance_path: Path,
    reconciliation_acceptance_path: Path,
    ...
    if not normalized_route_signal:
        raise ValueError("operational hardening route_signal must be non-empty")

    raw_ingest_report = _json_load(raw_ingest_report_path)
    canonicalization_report = _json_load(canonicalization_report_path)
    reconciliation_report = _json_load(reconciliation_report_path)
    raw_ingest_acceptance = _json_load(raw_ingest_acceptance_path)
    canonicalization_acceptance = _json_load(canonicalization_acceptance_path)
    reconciliation_acceptance = _json_load(reconciliation_acceptance_path)

    scheduler_policy = load_operational_scheduler_policy(scheduler_policy_path)
    monitoring_policy = load_operational_monitoring_policy(monitoring_policy_path)

    gate_a = _validate_raw_ingest_gate(
        report=raw_ingest_report,
        path=raw_ingest_report_path,
        acceptance_report=raw_ingest_acceptance,
        acceptance_path=raw_ingest_acceptance_path,
    )
    gate_b = _validate_capability_status_gate(
        gate="B",
        name="Canonical + Resampling",
        report=canonicalization_report,
        path=canonicalization_report_path,
        acceptance_report=canonicalization_acceptance,
        acceptance_path=canonicalization_acceptance_path,
    )
    gate_c = _validate_capability_status_gate(
        gate="C",
        name="Reconciliation",
        report=reconciliation_report,
        path=reconciliation_report_path,
        acceptance_report=reconciliation_acceptance,
        acceptance_path=reconciliation_acceptance_path,
    )
    ...
    checklist = [
        ...
        },
        {
            ...
            "evidence_path": "capability-gate-results",
        },
        {
    ...
    release_bundle = {
        ...
        "upstream_reports": {
            "raw_ingest": raw_ingest_report_path.resolve().as_posix(),
            "canonical_refresh": canonicalization_report_path.resolve().as_posix(),
            "reconciliation": reconciliation_report_path.resolve().as_posix(),
        },
        "upstream_acceptance_artifacts": {
            "raw_ingest": raw_ingest_acceptance_path.resolve().as_posix(),
            "canonical_refresh": canonicalization_acceptance_path.resolve().as_posix(),
            "reconciliation": reconciliation_acceptance_path.resolve().as_posix(),
        },
        "operations_evidence": {
    ...
    for report in (raw_ingest_report, canonicalization_report, reconciliation_report):
    ...
    report = {
        ...
        "input_paths": {
            "raw_ingest_report": raw_ingest_report_path.resolve().as_posix(),
            "canonical_refresh_report": canonicalization_report_path.resolve().as_posix(),
            "reconciliation_report": reconciliation_report_path.resolve().as_posix(),
            "raw_ingest_acceptance": raw_ingest_acceptance_path.resolve().as_posix(),
            "canonical_refresh_acceptance": canonicalization_acceptance_path.resolve().as_posix(),
            "reconciliation_acceptance": reconciliation_acceptance_path.resolve().as_posix(),
