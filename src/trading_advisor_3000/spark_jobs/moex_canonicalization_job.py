    ...
    *,
    ...
    try:
        if source_row_count == 0 or selected_interval_row_count == 0:
            ...
            annotated = joined.withColumn("rn_first", F.row_number().over(first_window)).withColumn(
                "rn_last", F.row_number().over(last_window)
            )
            ...
            aggregated = annotated.groupBy(*partition_columns).agg(
                ...
                F.max(F.when(F.col("rn_last") == 1, F.col("open_interest")))
                .cast("long")
                .alias("open_interest"),
                F.max(F.when(F.col("rn_last") == 1, F.col("source_provider"))).alias(
                    "source_provider"
                ),
                F.max(F.when(F.col("rn_last") == 1, F.col("source_timeframe"))).alias(
                    "source_timeframe"
                ),
                F.max(F.when(F.col("rn_last") == 1, F.col("source_interval")))
                .cast("int")
                .alias("selected_source_interval"),
                F.max(F.when(F.col("rn_last") == 1, F.col("source_run_id"))).alias("source_run_id"),
                F.max(F.when(F.col("rn_last") == 1, F.col("source_ingest_run_id"))).alias(
                    "source_ingest_run_id"
                ),
                F.count(F.lit(1)).cast("int").alias("source_row_count"),
                F.min(F.col("ts_open")).alias("source_ts_open_first"),
                F.max(F.when(F.col("rn_last") == 1, F.col("ts_close"))).alias(
                    "source_ts_close_last"
                ),
                F.max(F.col("open_interest_imputed").cast("int"))
                .cast("int")
