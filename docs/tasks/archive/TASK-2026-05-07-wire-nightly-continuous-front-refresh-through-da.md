...
- Closure Evidence: integration test coverage confirms the canonical dataset handoff into downstream research, while product-plane Dagster definitions, scheduled continuous-front policy selection, Spark-native calendar-expiry behavior, campaign schema, and live Dagster schedule/sensor checks cover the runtime-ready surface.
- Shortcut Waiver: none
- Design Checkpoint: chosen path=integrate nightly automation with the merged calendar/session active-front contour from main; why_not_shortcut=the branch wires the real Dagster schedule/sensor and keeps Spark/data contracts executable instead of documenting a manual refresh; future_shape=nightly MOEX baseline remains the trigger and research data prep refreshes continuous-front tables automatically after the baseline succeeds.
...
- Switched scheduled continuous-front refresh to the accepted calendar-expiry/session-active policy and retained the newer Spark-native active-front contour from `main`.
...
- `py -3.11 -m pytest tests/product-plane/unit/test_continuous_front_spark_job.py tests/product-plane/unit/test_research_campaign_runner.py tests/product-plane/integration/test_product_plane_dagster_definitions.py tests/product-plane/integration/test_research_dagster_jobs.py tests/product-plane/contracts/test_research_campaign_contracts.py -q --basetemp=.tmp/ptw2`
