...
2. Canonical builder deduplicates by `(contract_id, timeframe, ts_open)`.
3. Quality gate enforces whitelist membership and monotonic timeline.
4. Delta schemas are represented as a deterministic manifest in code.
5. Dagster/Spark artifacts are delivered as executable skeletons without external runtime dependency.
