...
| StockSharp/QUIK/Finam real broker process | implemented | Governed `F1-E` includes replayable Finam-native session preflight (`TA3000_FINAM_API_BASE_URL + /v1/sessions/details` with real JWT) and staging-real sidecar lifecycle evidence (`artifacts/f1/phase05/real-broker-process/20260331T184215Z-7a1dc827e46e/manifest.json`). | This does not by itself unlock full production readiness; contour proof must still be replayed after secret rotation or transport regressions. |
