...
- Target-state route ownership is fixed to `Dagster -> Python MOEX extraction/staging -> Spark raw Delta ingest -> Spark canonical refresh`.
...
- `Strategy Evaluation Layer v1` is the current post-backtest product layer that emits `strategy_evaluation_profile.v1`; `policy_pass`, projection eligibility, paper-signal readiness, paper-trade readiness, and `live-candidate` are fields in one evaluation output. `policy_pass` alone is not promotion evidence.
