...
Formula calculation is owned by pandas-ta-classic for base indicators and pandas window logic for derived indicators. Delta tables are the durable output surface; Dagster coordinates refresh and QC. Native volume/OI windows are masked or reset across roll boundaries, and derived levels are computed from causal normalized state before being shifted only to the current bar's known roll anchor.
...
Acceptance is anti-bypass: the publish decision requires actual QC observation rows for prefix invariance, independent formula samples, pandas-ta parity, lineage, schema, rule coverage, input projection, and adapter authorization. Missing required QC groups create a blocker `anti_bypass` observation and quarantine the run.

The independent formula gate covers every active base output from `default_indicator_profile()` and every active derived output from `current_derived_indicator_profile()`. The TZ minimum classes such as `sma_20`, `ema_20`, `atr_14`, `rsi_14`, `ppo_12_26_9`, `session_vwap`, native volume/OI relationships, levels, positions, cross codes, movement-angle divergence, angle slopes, and MTF overlays are a floor, not the gate boundary.

Prefix invariance uses selected cut timestamps around roll boundaries and the tail. Each cut rebuilds input, base sidecar, and derived sidecar rows from prefix-only bars and prefix-only ladder rows, then compares row hashes, null masks, and cross-window evidence through the cut.
