# Review and Observability Observability Runbook

## Purpose
Operate and validate Review and Observability review analytics and observability:
`signal events + fills + positions -> outcomes -> review metrics -> Prometheus/Loki exports -> Grafana views`.

## Preconditions
- Review and Observability tests are green.
...
1. Run Review and Observability integration tests:
   - `python -m pytest tests/product-plane/integration/test_review_observability.py -q`
2. Run Review and Observability unit tests:
   - `python -m pytest tests/product-plane/unit/test_review_metrics.py -q`
   - `python -m pytest tests/product-plane/unit/test_review_latency_metrics.py -q`
   - `python -m pytest tests/product-plane/unit/test_observability_export.py -q`
   - `python -m pytest tests/product-plane/unit/test_observability_deployment.py -q`
...
3. In Grafana, confirm dashboard `TA3000 Observability Overview` is provisioned.
...
2. Re-run integrated replay and regenerate Review and Observability artifacts.
3. Compare `review_report.summary.latency_status_counts` against previous known-good baseline.
