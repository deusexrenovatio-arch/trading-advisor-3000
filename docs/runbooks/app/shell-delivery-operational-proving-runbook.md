# Shell Delivery Operational Proving Operational Proving Runbook
...
## Preconditions
- Shell Delivery Operational Proving process tests are green.
...
1. Preview lane plan without side effects:
   ...
   - if report persistence is needed for review, add: `--write-dry-run-report --output artifacts/shell-delivery-operational-proving.dry-run.json`.
2. Run full proving:
...
3. Review report:
   - `artifacts/shell-delivery-operational-proving.json`
...
- Skip nightly lane for fast lane-only smoke:
  - `python scripts/run_shell_delivery_operational_proving.py --skip-nightly-lane --from-git --git-ref HEAD`
- Skip dashboard refresh for gate-only smoke:
  - `python scripts/run_shell_delivery_operational_proving.py --skip-dashboard-refresh --from-git --git-ref HEAD`
- Enforce lifecycle session checks in loop/pr steps:
  - `python scripts/run_shell_delivery_operational_proving.py --enforce-session-check --from-git --git-ref HEAD`
